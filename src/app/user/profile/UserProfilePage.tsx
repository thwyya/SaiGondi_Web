...
export default function UserProfilePage({ id }: { id?: string }) {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (id) {
          ...
          res = await authApi.getUserById(id);
        } else {
          // My profile (theo token)
          const token = localStorage.getItem('accessToken');
          if (!token) return;
          res = await authApi.getProfile(token);
        }
        setUser(res.user || res);
        setFilteredBlogs(res.user?.blogs || res.blogs || []);
      } catch (err) {
  ...
  const handleBanAccount = async () => {
    if (!id && confirm("Bạn có chắc muốn khóa tài khoản của mình?")) {
      try {
        ...
        >
          ...
          <div>
            ...
            <div className="m-4 shadow p-4 rounded-md bg-white">
              ...
              <PostTable data={filteredBlogs} showActions={!id} />
            </div>
          </div>
        </div>
      </div>
