...
export default function ProfileContainer({ isOwnProfile = false }: { isOwnProfile?: boolean }) {
  const { id } = useParams(); 
    ...
    const fetchUser = async () => {
      try {
        ...
        if (isOwnProfile) {
          ...
          if (!token) return;
          res = await authApi.getProfile(token);
        } else if (id) {
          res = await authApi.getUserById(id as string);
        }
  ...
  return (
    <>
      ...
      <div className="flex flex-col">
        <div className="relative m-4 md:m-8 w-[95%] mx-auto h-[200px] md:h-[300px] rounded-3xl bg-[#307AFD]">
          ...
          <span className="absolute top-4 left-4 bg-[#FFFFFF4D] px-4 py-2 rounded-lg text-lg md:text-xl text-black">
            {isOwnProfile ? "My Data" : "User Data"}
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-[30%_70%] w-[92%] md:w-[84%] mx-auto gap-4 -mt-2 pb-10">
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded-sm shadow">
              ...
              <div className="flex gap-4 items-center w-[95%] mx-auto mb-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <span className="block text-sm text-gray-700 break-words">{user.email}</span>
                </div>
              </div>
            ...
            {user.badges && user.badges.length > 0 && (
              <div className="bg-white rounded-sm shadow p-4">
                <h3 className="font-semibold mb-3 text-center">Huy hiệu đã đạt được</h3>
                <div className="flex flex-wrap gap-2 justify-center">
                  {(showAllBadges ? user.badges : user.badges.slice(0, 3))
          ...
          <div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 md:gap-6 p-4 mb-6">
              <CardItem icon="ri-navigation-line" label="Điểm đến" count={user.checkinCount} growth={user.checkinGrowth} color="#0D894F" />
              <CardItem icon="ri-file-text-fill" label="Bài viết" count={user.blogCount} growth={user.blogGrowth} color="#E46A11" />
              <CardItem icon="ri-verified-badge-line" label="Bài đánh giá" count={user.reviewCount} growth={user.reviewGrowth} color="#4338CA" />
            </div>
            <div className="m-4 shadow p-4 rounded-md bg-white">
              ...
              <PostTable data={filteredBlogs} showActions={isOwnProfile} />
            </div>
          </div>
        </div>
      </div>
